...
In GitHub repository settings, add the secrets `MAC_CSC_LINK` (the base64-encoded Developer ID `.p12` certificate), `MAC_CSC_KEY_PASSWORD`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, and `APPLE_TEAM_ID`. macOS release builds require signing and fail early when credentials are missing. Keep credentials out of source files and release notes.

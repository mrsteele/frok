...
The desktop app uses `~/frok/data/library/`. Browser development uses `.data/library/`; desktop development uses its separate development workspace. Each library contains `frok.sqlite`, `media/`, `jobs/`, and temporary deletion staging. Model workspaces, downloaded runtimes and pipeline folders remain separate.

**Delete all my stuff** requires confirmation, cancels queued/active work, waits for file operations to settle and removes library media, job files, records and local export archives. It resets app settings, recipes and local interface preferences. Other open windows clear their stale preferences when they receive the new reset marker. Downloaded models, runner installations, pipeline definitions, saved API tokens and administrator environment files are kept. Remove saved tokens separately in **Settings → Generate → API tokens**.
...
| Automatic job cleanup | Removes finished job records, logs and working files after the configured period (3 hours by default); saved media stays. |
...
If reference uploads finish but the generation is never queued, unused attachments are cleaned up after 24 hours while Frok is open, even when automatic job cleanup is off. A draft still open in the browser can upload its local file again on retry. Fresh uploads and references used by any asset or job are protected.
...
Configure automatic cleanup in **Settings → Generate → Generation preferences**. The period starts when a job finishes and includes completed, failed and cancelled jobs. Turn it off to keep details until you delete them. Cleanup runs about once a minute while Frok is open and catches up at startup.
...
In **Settings → Generate**, choose **Export all my data** above Delete all my stuff. Frok prepares a `.tar.gz` archive and starts a normal download; the desktop app lets you choose where to save it.
...
Large libraries need temporary disk space for the prepared archive, plus space at the download destination. The archive is streamed from disk rather than loaded into page memory. When you request an export, Frok keeps a local archive so **Download again** is available for 24 hours. Expired archives are removed on the next export, so they may remain on disk longer if you do not export again. **Delete all my stuff** removes these archives immediately as part of a successful reset. Your downloaded archive is independent of that reset; no archive is uploaded to Frok's developers.

**Verify the download has finished and the archive opens before deleting anything.** Automatic import is not available yet. The included README explains manual recovery with Frok fully stopped; do not merge SQLite database/WAL files. Media files can also be opened directly without Frok. Recipes and interface preferences come from the window that started the export.

## Copy a complete workspace

Quit Frok before copying the workspace so SQLite and media files are consistent. For a packaged installation, copy `~/frok/data` and `~/frok/desktop-profile` together to preserve the library and local interface preferences. Also keep `~/frok/pipelines` and any custom pipeline location. Back up runner model folders separately if you want to avoid downloading them again.

For web development, the library is under the repository's `.data`; interface preferences are stored by the local browser origin. Desktop development uses `.data/desktop-dev`. These development libraries are separate from the packaged app's default library.

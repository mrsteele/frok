...
import { copySharpWindowsRuntime, checkDesktopSharp } from './desktop-native.mjs';
...
await copySharpWindowsRuntime(root, backend);
...
console.log(await checkDesktopSharp(backend, path.join(output, 'runtime', process.platform === 'win32' ? 'node.exe' : 'node')));

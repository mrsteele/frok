...
async function smoke() {
  ...
  try {
    ...
    const setMotionDraft = text => js(`(()=>{const prompt=document.querySelector('#viewer-motion-prompt');Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,'value').set.call(prompt,${JSON.stringify(text)});prompt.dispatchEvent(new Event('input',{bubbles:true}));})()`);
    for (const width of [420, 1280]) {
      window.setContentSize(width, 900);
      await load(assetPath);
      assert.equal(await js("document.querySelector('#viewer-motion-prompt').value"), '', 'image assets open with a blank custom video prompt');
      assert.equal(await js("document.querySelector('#viewer-motion-prompt').placeholder"), 'Write a custom video prompt…');
      assert.equal(await js("document.querySelector('.viewer-default-recipe')"), null, 'custom editor has no default recipe hint');
      assert.equal(await js("document.querySelector('.viewer-render').disabled"), true, 'empty custom prompt cannot generate');
      await setMotionDraft('Keep this custom motion draft');
      await wait("document.querySelector('#viewer-motion-prompt').value==='Keep this custom motion draft'");
      await js("document.querySelector('.viewer-recipes summary').focus()");
      key('Enter');
      await wait("document.querySelector('.viewer-recipes').open");
      assert.equal(await posts(), 0, 'opening the recipe menu does not generate');
      await noOverflow('recipe menu ' + width);
      await shot('asset-recipe-menu-' + width);
      const recipe = await js("(()=>{const button=Array.from(document.querySelectorAll('.viewer-recipes button')).find(el=>el.querySelector('strong').textContent==='Dance');button.focus();return {prompt:button.querySelector('small').textContent};})()");
      if (width === 420) key('Enter');
      else await js("(()=>{const button=document.activeElement;button.click();button.click();})()");
      await wait("window.__uiCalls.some(call=>call.url==='/api/jobs' && call.method==='POST')");
      assert.equal(await js("Array.from(document.querySelectorAll('.viewer-recipes button')).every(button=>button.disabled) && document.querySelector('.viewer-render').disabled"), true, 'recipe and generate actions disable while submitting');
      await js("document.querySelector('.viewer-recipes button').click();document.querySelector('.viewer-render').click()");
      await shot('asset-recipe-queuing-' + width);
      await wait("!!document.querySelector('.viewer-queue') && !document.querySelector('.viewer-render').disabled");
      assert.equal(await posts(), 1, 'choosing a recipe queues exactly one job without a Generate click');
      const recipeRequest = await js("window.__uiCalls.find(call=>call.url==='/api/jobs' && call.method==='POST').body");
      assert.equal(recipeRequest.sourceId, assetPath.split('/').at(-1));
      assert.equal(recipeRequest.mode, 'video');
      assert.equal(recipeRequest.count, 1);
      assert.equal(recipeRequest.videoStyle, 'preset');
      assert.equal(recipeRequest.videoPreset.id, 'dance', 'uses the activated recipe instead of the default');
      assert.equal(recipeRequest.videoPreset.prompt, recipe.prompt, 'snapshots the complete recipe');
      assert.equal(recipeRequest.prompt, '', 'recipe generation excludes the custom draft');
      assert.equal(await js("document.querySelector('#viewer-motion-prompt').value"), 'Keep this custom motion draft');
      assert.equal(await js("document.querySelector('.viewer-recipes').open"), false, 'recipe menu closes after activation');
      assert.equal(await js("document.activeElement===document.querySelector('.viewer-recipes summary')"), true, 'focus returns to the recipe trigger');
      assert.equal(await js("document.querySelector('.viewer-recipes summary').textContent.trim()"), 'Generate with recipe');
      assert.equal(await js("document.querySelector('.viewer-recipes [aria-pressed]')===null"), true, 'recipes are actions rather than persistent selections');
      assert.equal(await js('location.pathname'), assetPath, 'recipe generation stays on the asset');
      await noOverflow('recipe queued ' + width);
      await shot('asset-recipe-queued-' + width);
    }

    await load(assetPath);
    for (const draft of ['', '  \n  ']) {
      await setMotionDraft(draft);
      await js("document.querySelector('#viewer-motion-prompt').focus()");
      key('Enter', [process.platform === 'darwin' ? 'meta' : 'control']);
      await js("document.querySelector('.viewer-render').click()");
      await pause(120);
      assert.equal(await posts(), 0, 'blank custom prompts never submit or fall back to the default recipe');
      assert.equal(await js("document.querySelector('.viewer-render').disabled"), true);
      assert.equal(await js("Array.from(document.querySelectorAll('.viewer-recipes button')).every(button=>!button.disabled)"), true, 'recipes stay available without custom text');
    }
    await setMotionDraft('');
    await js("document.querySelector('.viewer-recipes summary').click();document.querySelector('.viewer-recipes button[aria-label=\"Generate video using Dance\"]').click()");
    await wait("!!document.querySelector('.viewer-queue') && !document.querySelector('.viewer-recipes button').disabled");
    assert.equal(await posts(), 1, 'recipe action works with an empty custom prompt');
    assert.equal(await js("window.__uiCalls.find(call=>call.url==='/api/jobs' && call.method==='POST').body.videoPreset.id"), 'dance');
    assert.equal(await js("document.querySelector('#viewer-motion-prompt').value"), '', 'using a recipe keeps the custom field blank');
    assert.equal(await js("document.querySelector('.viewer-render').disabled"), true, 'custom Generate still requires its own prompt after a recipe');

    await load(assetPath, 'error');
    await setMotionDraft('Keep this draft after a queue error');
    await js("document.querySelector('.viewer-recipes summary').click();document.querySelector('.viewer-recipes button').click()");
    await wait("!!document.querySelector('.viewer-error[role=alert]') && !document.querySelector('.viewer-render').disabled");
    assert.equal(await posts(), 1, 'failed recipe submission is attempted once');
    assert.equal(await js("document.querySelector('#viewer-motion-prompt').value"), 'Keep this draft after a queue error');
    assert.equal(await js("document.activeElement===document.querySelector('.viewer-recipes summary')"), true);
    await shot('asset-recipe-error-1280');
    await js("document.querySelector('.viewer-recipes summary').click();document.querySelector('.viewer-recipes button').click()");
    await wait("window.__uiCalls.filter(call=>call.url==='/api/jobs' && call.method==='POST').length===2 && !document.querySelector('.viewer-render').disabled");

    await load(assetPath, 'missing');
    await js("document.querySelector('.viewer-recipes summary').click()");
    assert.equal(await js("Array.from(document.querySelectorAll('.viewer-recipes button')).every(button=>button.disabled)"), true, 'missing dependencies disable recipe actions');
    await js("document.querySelector('.viewer-recipes button').click()");
    assert.equal(await posts(), 0, 'blocked recipes do not submit');

    window.setContentSize(420, 900);
    await load(assetPath + '/2', 'video-take');
    assert.equal(await js("document.querySelectorAll('.viewer-generation-actions button').length"), 3, 'video take exposes Upscale, Redo and Generate');
    assert.equal(await js("document.querySelector('#viewer-motion-prompt').value"), 'Clouds drift over the lake');
    await js("document.querySelector('.viewer-composer').scrollIntoView({block:'center'});document.querySelector('.viewer-recipes summary').click()");
    await noOverflow('video take recipe menu 420');
    await shot('asset-video-recipe-menu-420');
    await js("document.querySelector('.viewer-recipes button[aria-label=\"Generate video using Dance\"]').click()");
    await wait("!!document.querySelector('.viewer-queue') && !document.querySelector('.viewer-render').disabled");
    assert.equal(await posts(), 1, 'a recipe on an existing take immediately queues another video');
    assert.equal(await js("window.__uiCalls.find(call=>call.url==='/api/jobs' && call.method==='POST').body.sourceId"), assetPath.split('/').at(-1), 'recipe on a video take retains the root image');
    assert.equal(await js("document.querySelector('#viewer-motion-prompt').value"), 'Clouds drift over the lake', 'recipe does not replace the previous take’s displayed motion');
    await noOverflow('video take recipe queued 420');
    await shot('asset-video-recipe-queued-420');
    await setMotionDraft('  ');
    await js("Array.from(document.querySelectorAll('.viewer-generation-actions button')).find(button=>button.textContent.includes('Redo')).click()");
    await wait("window.__uiCalls.filter(call=>call.url==='/api/jobs' && call.method==='POST').length===2 && !document.querySelector('.viewer-recipes button').disabled");
    assert.equal(await js("window.__uiCalls.filter(call=>call.url==='/api/jobs' && call.method==='POST').at(-1).body.prompt"), 'Clouds drift over the lake', 'Redo uses the saved direction even with an empty custom draft');
    window.setContentSize(1280, 900);

    await load(assetPath);
    await setMotionDraft('Sail slowly');
    await js("document.querySelector('#viewer-motion-prompt').focus()");
    key('Enter');
    await pause(120);
    ...
    const customRequest = await js("window.__uiCalls.find(call=>call.url==='/api/jobs' && call.method==='POST').body");
    assert.equal(customRequest.videoStyle, 'custom');
    assert.equal(customRequest.videoPreset, undefined);
    assert.equal(customRequest.prompt.trim(), 'Sail slowly');
    ...
    console.log('Whole-app UX checks passed: 420/800/1280px, 200% zoom, visible navigation, immediate recipe generation, preserved custom drafts, loading/error recovery, keyboard, queue, deletion review, onboarding recovery and reduced motion. Synthetic data only.');
  } catch (error) {
    console.error(error);
    code = 1;

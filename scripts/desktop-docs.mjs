...
async function buildDocs(web = false) {
  ...
  await new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [cli, 'build', '--outDir', output], { cwd:docs, stdio:'inherit', env:{ ...process.env, DOCS_BASE:web ? '/docs/' : '/', DOCS_SITE_URL:'', DOCS_APP_VERSION:version } });
...
export const buildWebDocs = () => buildDocs(true);

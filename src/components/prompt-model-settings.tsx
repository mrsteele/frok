...
export function PromptModelSettings({health,jobs,checking,onRefresh,onPrepare,embedded=false,anchor=true,enhance=false,onEnhancementChange,onBusyChange}:{health?:Health;jobs:Job[];checking:boolean;onRefresh:()=>void;onPrepare:(task:string)=>Promise<void>;embedded?:boolean;anchor?:boolean;enhance?:boolean;onEnhancementChange?:(enabled:boolean)=>void;onBusyChange?:(busy:boolean)=>void}) {
  ...
  async function choose(model:string) {
    ...
    try{await api('settings','PATCH',{modelSelections:{prompt:model==='__off__'?null:model}});await onRefresh();}
    catch(error){setError((error as Error).message);setValue(saved);}
    finally{setBusy(false);}
  }

  return <section id={anchor?'setup-prompt':undefined} className={embedded?'ollama-model-settings':'service-card model-choice'}>
    ...
    {!connected&&<p className="settings-hint">Connect Ollama to choose a model. <Link href="/settings?service=ollama">Configure connection →</Link></p>}
    ...
    {job?<p className="settings-working"><Loader2 size={13} className="spin"/>{job.message} <Link href={`/queue/${job.id}`}>View job →</Link></p>
      :connected&&needsDownload&&<footer><p className="settings-hint">Download {effective}. Enhancement becomes available once the selected model is installed.</p>

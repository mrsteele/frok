...
type Bundle={base64:string;filename:string;folder:string;unresolved:string[]};
...
export function PipelineUtils() {
  const [name,setName]=useState(''),[kind,setKind]=useState('image'),[runner,setRunner]=useState('vpipe');
  const [workflow,setWorkflow]=useState<Record<string,unknown>>(),[reading,setReading]=useState(false);
  const [busy,setBusy]=useState(false),[error,setError]=useState(''),[result,setResult]=useState<Bundle>();
  const lock=useRef(false),upload=useRef(0);
  async function read(file:File|undefined) {
    const revision=++upload.current;
    setWorkflow(undefined);setResult(undefined);setError('');setReading(false);
    if(!file)return;
    if(file.size>1_000_000){setError('Choose a workflow under 1 MB.');return;}
    setReading(true);
    try {
      const graph=JSON.parse(await file.text());
      if(revision!==upload.current)return;
      if(!graph||typeof graph!=='object'||Array.isArray(graph))throw Error('The workflow must be a JSON object.');
      setWorkflow(graph);
    }catch(error){if(revision===upload.current)setError(error instanceof SyntaxError?'The workflow is not valid JSON.':(error as Error).message);}
    finally{if(revision===upload.current)setReading(false);}
  }
  async function download() {
    if(lock.current||!workflow||!name.trim())return;
    lock.current=true;setBusy(true);setError('');setResult(undefined);
    try {
      const bundle=await api<Bundle>('utils/pipeline','POST',{name,kind,runner,graph:workflow});
      const bytes=Uint8Array.from(atob(bundle.base64),c=>c.charCodeAt(0));
      save(new Blob([bytes],{type:'application/zip'}),bundle.filename);setResult(bundle);
    }catch(error){setError((error as Error).message);}
    finally{lock.current=false;setBusy(false);}
  }
  return <div className="pipeline-utils">
    <section className="service-card" aria-labelledby="pipeline-builder-title">
      <header><div><h3 id="pipeline-builder-title">Create a pipeline folder</h3><p>Turn a workflow into a Frok pipeline. Download its run file, preparation companion and metadata together in one ZIP.</p></div><FolderOpen size={21}/></header>
      <form onSubmit={event=>{event.preventDefault();void download();}}>
        ...
        <footer><button type="submit" className="settings-button" disabled={busy||reading||!workflow||!name.trim()}><Download size={14}/>{busy?'Building folder…':reading?'Reading workflow…':'Download pipeline folder'}</button></footer>
      </form>
      <p className="settings-hint">Supported inputs and known downloads are mapped automatically. Anything that needs manual work is listed in REVIEW.txt. Nothing is run or installed.</p>

...
export function useComposer() {
  ...
  useEffect(() => {
    try {
      setRequest(freshComposer(readInterfacePreference(composerPreferencesKey)));
    } catch {
      setStorageError(
        'Local preferences are unavailable. These settings will last only until you close Frok.',
      );
    }
    try {
      sessionStorage.removeItem('frok-last');
      sessionStorage.removeItem('frok-session-jobs');
    } catch {}
    setRestored(true);
  }, []);
  ...
  useEffect(() => {
    ...
    if (!restored) return;
    let closed = false;
    void writeInterfacePreference(composerPreferencesKey, preferences)
      .then(() => {
        if (!closed) setStorageError('');
      })
      .catch(() => {
        if (!closed)
          setStorageError(
            'Could not save your settings. These choices will last only until you close Frok.',
          );
      });
    return () => {
      closed = true;

...
import {
  applyDefaultVideoPreset,
  assignDefaultVideoPreset,
  decodeVideoPresets,
  defaultVideoPreset,
  needsDefaultVideoPreset,
  normalizeVideoPresetDefaults,
  videoPresetsKey,
...
export function useVideoPresets() {
  ...
  useEffect(() => {
    const refresh = () => {
      try {
        setState({ presets: read(), ready: true, error: '' });
      } catch (e) {
        setState({
          presets: [],
          ready: true,
    ...
    const unsubscribe = subscribeInterfacePreferences(refresh);
    refresh();
    return unsubscribe;
  }, []);
  return {
    ...
    setDefaultPreset(id: string) {
      return write(assignDefaultVideoPreset(read(), id));
    },
    ...
    resetPresets() {
      return write(decodeVideoPresets(null));
    },
  };
}

...
export function useDraftImages(sourceId: string | undefined, referenceIds: string[]) {
  ...
  useEffect(() => {
    ...
    drafts.current = drafts.current.filter((item) => ids.has(item.id));
    setItems(drafts.current);
  }, [selected]);
  useEffect(
    () => () => {
      drafts.current.forEach((item) => URL.revokeObjectURL(item.previewUrl));
      drafts.current = [];
    },
    [],
  );
  const stage = useCallback((files: File[]) => {
    ...
    drafts.current = [...drafts.current, ...added];
    setItems(drafts.current);
    return added;
  }, []);
  return { items, stage };

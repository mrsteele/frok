    ...
    async function poll() {
      ...
      try {
        const response = await fetch(
          `/api/jobs/${encodeURIComponent(jobId)}/preview?index=${index}`,
          {
        ...
        if (
          ...
          while (urls.size > 2) {
            const old = urls.values().next().value!;
...
export function PendingCard({
  ...
  entry: Extract<GalleryEntry, { kind: 'pending' }>;
  onQueue: () => void;
  children?: ReactNode;
}) {
  ...
  const preview = useImagePreview(
    entry.job.id,
    entry.index,
    generating && image && entry.job.runner === 'vpipe',
  );
  const step = entry.job.step;
  const percent =
    generating && step && step.total > 0
      ? Math.max(0, Math.min(100, Math.round((step.current / step.total) * 100)))
      : undefined;
  return (
    <article
      className={`media-card pending-card ${entry.state} ${preview ? 'has-live-preview' : ''}`}
      style={{ aspectRatio: entry.aspect }}
      aria-label={`${image ? 'Image' : 'Video'} ${entry.index + 1} of ${entry.job.total}: ${entry.state}`}
      aria-busy="true"
    >
      {preview && (
        <>
          <img
            ...
            src={preview}
            alt={`Live preview of image ${entry.index + 1} taking shape`}
          />
          <div className="pending-preview-shade" />
          <span className="live-preview-badge">Live preview</span>
        </>
      )}
      <div className="pending-content">
        ...
        <strong>
          {generating
            ? `Generating${percent === undefined ? '' : ` · ${percent}%`}`
            : entry.state === 'loading'
              ? 'Loading image…'
              : 'Queued'}
        </strong>

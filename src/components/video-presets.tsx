...
export function VideoPresets() {
  ...
  async function save() {
    ...
    try { await savePreset(draft); setDraft(undefined); setNotice('Preset saved. Choose it in the video prompt menu.'); setFailure(''); }
  ...
  async function makeDefault(preset: VideoPreset) {
    if(lock.current)return;
    lock.current=true;setBusy(true);
    try { await setDefaultPreset(preset.id); setNotice(`${preset.name} is now the default for quick videos and empty prompts.`); setFailure(''); }
    catch (e) { setFailure((e as Error).message); }
    finally{lock.current=false;setBusy(false);}
  }
  async function reset() {
    if(lock.current)return;
    lock.current=true;setBusy(true);
    try { await resetPresets(); setResetting(false); setDraft(undefined); setDeleting(undefined); setFailure(''); setNotice('Starter recipes restored.'); }
    catch (e) { setFailure((e as Error).message); }
    finally{lock.current=false;setBusy(false);}
  }
  return <section className="motion-presets" aria-labelledby="motion-presets-title" aria-busy={busy}>
    <div className="preset-heading"><div><h3 id="motion-presets-title">Motion presets</h3><p>Reuse a direction across different images.</p></div><div className="preset-heading-actions"><button className="settings-button" disabled={busy||!ready} onClick={() => {setResetting(true);setDeleting(undefined);setFailure('');setNotice('');}}><RotateCcw size={13}/>Reset recipes</button><button className="settings-button" disabled={busy||!ready || !!error || !!draft} onClick={() => edit()}><Plus size={13}/>Add preset</button></div></div>
    ...
    {!ready ? <p className="settings-hint">Loading presets…</p> : !error && <div className="preset-list">{presets.map(preset => <div className="preset-row" key={preset.id}>
      ...
      <div className="preset-actions"><button type="button" className="settings-button preset-default" disabled={busy||!!draft} aria-pressed={!!preset.isDefault} aria-label={preset.isDefault ? `${preset.name} is the default recipe` : `Make ${preset.name} the default recipe`} onClick={() => makeDefault(preset)}><Star size={13} fill={preset.isDefault ? 'currentColor' : 'none'}/>{preset.isDefault ? 'Default' : 'Make default'}</button><button className="icon-button" disabled={busy||!!draft} aria-label={`Edit ${preset.name}`} onClick={() => edit(preset)}><Pencil size={15}/></button><button className="icon-button" disabled={busy||!!draft} aria-label={`Delete ${preset.name}`} onClick={() => { setDeleting(preset.id); setFailure(''); }}><Trash2 size={15}/></button></div>
      {deleting === preset.id && <ConfirmationDialog title={`Delete “${preset.name}”?`} onClose={() => setDeleting(undefined)}><p>This removes the saved preset. Existing renders will keep their recipe.</p>{preset.isDefault && <p>{presets.find(item => item.id !== preset.id) ? `“${presets.find(item => item.id !== preset.id)!.name}” will become the default recipe.` : 'You will need to type a video prompt or add a recipe before using quick video actions.'}</p>}{failure&&<p className="delete-warning" role="alert">{failure}</p>}<div className="delete-actions"><button className="secondary" autoFocus onClick={() => setDeleting(undefined)}>Cancel</button><button disabled={busy} className="delete-confirm" onClick={() => remove(preset.id)}>Delete preset</button></div></ConfirmationDialog>}
    ...
    {draft && <form className="preset-editor" onSubmit={e => { e.preventDefault(); save(); }}>
      ...
      <div className="preset-editor-actions"><button type="button" className="settings-button" onClick={() => { setDraft(undefined); setFailure(''); }}>Cancel</button><button className="settings-button solid" type="submit" disabled={busy}>Save preset</button></div>
    </form>}

...
export function SidebarQueue({
  ...
  queue: ReturnType<typeof queueView>;
  worker?: boolean;
  telemetry?: Telemetry;
  current: boolean;
  stopping: boolean;
  ...
  const job = running || pending[0],
    live = worker === true ? running : undefined,
  ...
  return (
    <div ref={container} className="sidebar-queue" data-state={state}>
      <button
        ref={toggle}
      ...
      <section
        id="sidebar-queue-details"
        className={`sidebar-queue-panel ${expanded ? 'expanded' : ''}`}
        aria-label="Generation queue status"
      >
        <Link
          className="queue-panel-heading"
          href="/queue"
          onClick={close}
          aria-label={`Open generation queue · ${label}`}
        ...
        <div className="queue-panel-body">
          <div className="queue-current">
            ...
            {running && (
              <button
                className="queue-stop icon-button"
                disabled={stopping}
                onClick={() => {
                  close();
                  onStop(running);
                ...
                aria-label="Stop current job"
              >
                {stopping ? <Loader2 size={14} className="spin" /> : <X size={15} />}
              </button>
            )}
          </div>

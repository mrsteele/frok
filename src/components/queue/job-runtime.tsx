...
export function JobRuntime({ job }: { job: Job }) {
  ...
  const duration = formatRunnerTime(
    elapsed ?? (job.status !== 'queued' ? job.runnerSeconds : undefined),
  );
  if (!duration)
    return job.status === 'queued' ? null : (
      <small
        className="job-runtime"
        title="This older job has no recorded start time or usable runner duration."
      >
        Time not recorded
      </small>
    );
  ...
  const title = reportedOnly
    ? `Rendering time reported by ${service}.`
    : `Total processing time, including prompt enhancement and saving; excludes queue wait.${runner ? ` ${service} rendering: ${runner}.` : ''}`;
  return (
    <small className="job-runtime" title={title}>
      {reportedOnly ? `${service} rendering` : job.status === 'running' ? 'Elapsed' : 'Ran for'} ·{' '}
      {duration}
    </small>
  );
}

...
Use task names in navigation: Create, Favorites, Settings, Docs and Queue. Call reusable motion prompts recipes, and generation configurations workflows. Reserve “load” and “show older” for existing data; generation actions must say Generate. Opening an asset or its recipe menu does not start a job. An image asset's video prompt starts blank for custom directions; custom Generate and its keyboard shortcut require non-whitespace text and never fall back to a recipe. On the asset page, choosing a saved recipe from Generate with recipe immediately queues a video using the image prompt and that recipe, preserving the custom text draft. These are action buttons, not persistent selections. The Create composer’s recipe selector still configures its next Generate request. Use Cmd/Ctrl+Enter to generate a custom prompt and leave plain Enter available for multiline prompts.

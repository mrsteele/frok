...
type State = { options: RuntimeOptions; restartRequired: boolean };
export function RuntimeSettings({ onRefresh, ollamaUrl }: { onRefresh: () => void; ollamaUrl?: string }) {
  const [state, setState] = useState<State>(), [draft, setDraft] = useState<RuntimeOptions>();
  const [busy, setBusy] = useState(false), [error, setError] = useState(''), [saved, setSaved] = useState(false);
  useEffect(() => { let closed = false; void api<State>('settings/runtime').then(value => { if (!closed) { setState(value); setDraft(current => current ?? value.options); } }).catch(error => { if (!closed) setError(error.message); }); return () => { closed = true; }; }, [ollamaUrl]);
  async function save() {
  ...
  return <section className="runtime-settings desktop-settings" aria-labelledby="runtime-settings-title">
    <h3 id="runtime-settings-title"><SlidersHorizontal size={17}/>Generation preferences</h3>
    {draft && <form onSubmit={event => { event.preventDefault(); void save(); }}>
      <fieldset disabled={busy}>
        <label id="video-tools-folder" className="settings-field">Video tools folder<input type="text" value={draft.mediaToolsDirectory} placeholder="Detect installed FFmpeg and FFprobe automatically" onChange={event => { setDraft({ ...draft, mediaToolsDirectory: event.target.value }); setSaved(false); }}/><small>Optional: the folder containing both commands. Frok uses your installation and does not download or bundle these tools. Save, then refresh connections to check it.</small></label>
        ...
        <label className="runtime-toggle"><input type="checkbox" checked={draft.manageOllama} onChange={event => { setDraft({ ...draft, manageOllama: event.target.checked }); setSaved(false); }}/><span>Let Frok start Ollama<small>Start an installed Frok-managed runtime when the app opens. Uses a separate model store and defaults to port 11435. Requires quitting and reopening Frok.</small></span></label>
    ...
    {state?.restartRequired && <p className="runtime-restart" role="status">Saved. Quit and reopen Frok to apply the Ollama startup changes.</p>}
    {saved && !changed && !state?.restartRequired && <p role="status">Preferences saved.</p>}

...
export function JobRuntime({ job }: { job: Job }) {
  ...
  const title = reportedOnly ? 'Rendering time reported by Vpipe.'
    : `Total processing time, including prompt enhancement and saving; excludes queue wait.${runner ? ` Vpipe rendering: ${runner}.` : ''}`;
  return <small className="job-runtime" title={title}>{reportedOnly ? 'Vpipe rendering' : job.status === 'running' ? 'Elapsed' : 'Ran for'} · {duration}</small>;
}

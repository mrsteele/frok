...
export function useComposer() {
  ...
  useEffect(()=>{
    try{setRequest(freshComposer(localStorage.getItem(composerPreferencesKey)));}
  ...
  useEffect(()=>{
    ...
    if(!restored)return;
    try{localStorage.setItem(composerPreferencesKey,preferences);setStorageError('');}
    catch{setStorageError('Could not save your settings. These choices will last only until you close Frok.');}

...
export const dependencySchema=z.object({
  kind:z.enum(['model','lora','file']),reference:relative,layout:z.enum(['krea','minimax','transformer']).optional(),
...
export const pipelineMetadata=z.object({
  version:z.literal(1),id:pipelineId,name:z.string().trim().min(1).max(100),runner:z.enum(['vpipe','comfyui','local']),
  ...
  bindings:z.partialRecord(z.enum(['prompt','seed','width','height','frames','duration','output','pixels']),z.array(binding)).default({}),
  source:z.object({target:z.string(),field:z.string().optional(),port:z.number().int().nonnegative().optional(),model:z.string().optional()}).optional(),
  ...
  engine:z.enum(['realesrgan','seedvr2']).optional(),
}).strict();

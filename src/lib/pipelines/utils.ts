...
export async function buildPipelineBundle(input:unknown) {
  ...
  // Only bundled companions can supply executable preparation. Reuse one only
  // when every model/adapter it prepares is actually requested by this graph.
  const companion=value.runner==='vpipe'&&deps.some(d=>d.generated)?factory.entries.find(p=>p.metadata.runner==='vpipe'&&p.prepare&&
    dependencies(p).some(d=>d.generated)&&dependencies(p).every(d=>deps.some(item=>item.reference===d.reference&&item.kind===d.kind))):undefined;
  const covered=new Set(companion?dependencies(companion).map(d=>d.reference):[]);
  if(value.runner==='vpipe') {
    const prepare=companion?structuredClone(companion.prepare!):{stages:[]};
    const stages=prepare.stages as Record<string,unknown>[];
    const fetches=new Set<string>();
    for(const d of deps) {
      if(covered.has(d.reference))continue;
      if(!d.fetch||d.generated) {
        unresolved.push(`${d.reference}: ${d.generated?'requires custom fusion or quantization instructions':'download source is unknown'}.`);continue;
      }
      const fetchKey=JSON.stringify(d.fetch);if(fetches.has(fetchKey))continue;fetches.add(fetchKey);
      let id=`utility-fetch-${fetches.size}`;while(stages.some(s=>s.id===id))id+='-next';
      stages.push({id,type:'model-fetch',config:{model_path:d.fetch.model,...(d.fetch.variant?{model_variant:d.fetch.variant}:{}),...(d.fetch.key?{model_key:d.fetch.key}:{}),base_path:'./models',skip_existing_files:true,overwrite_existing:false}});
    }
    snapshot.prepare={...prepare,id:`prepare-${slug}`,description:'Preparation draft generated by Frok. Review REVIEW.txt before use.',stages,subpipelines:[]};
  }else {
    for(const d of deps)if(!d.url)unresolved.push(`${d.reference}: download source is unknown.`);
    snapshot.prepare={dependencies:deps};
  }
  ...
  const review=[
    ...
    'Unknown downloads are not guessed. Add verified sources to meta.json and prepare, and install any ComfyUI custom nodes separately. Vpipe prepare files contain native stages; ComfyUI prepare files are dependency manifests.',
  ...
  const files:UtilityFile[]=[
    ...
    {name:`${folder}/prepare.${extension}`,content:serialize(snapshot.prepare)},

...
import { upscalerSupported } from './upscale';
...
export async function validateServiceSettings(input:Change,signal:AbortSignal) {
  ...
  const modelSelections={...current.modelSelections,prompt:current.promptModelSetting??null,
    ...
    ...(input.ollamaModel?{prompt:input.ollamaModel}:{}),...(input.upscaler?{upscale:input.upscaler}:{}),...input.modelSelections};
  ...
  for(const capability of changed){
    ...
    if(connection&&!connections[connection])throw new HttpError(409,`Configure ${connectionNames[connection]} in Settings → Generate before choosing its models.`);
  }
  ...
  if(changed.includes('upscale')&&!upscalerSupported(modelSelections.upscale!))throw new HttpError(400,'That upscaler is not supported on this platform.');
...
export async function checkSetupConnection(task:string) {
  if(['runtime','ollama-runtime'].includes(task))return;
  const s=settings();
  if(['upscale','seedvr2'].includes(task))return;
  if(task!=='ollama')throw new HttpError(400,'Choose a pipeline in Settings → Pipelines to prepare its dependencies.');
  const connection='ollama';
  if(connection){
    if(!s.connections[connection])throw new HttpError(409,`Configure ${connectionNames[connection]} in Settings → Generate first.`);

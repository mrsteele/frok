...
export const runtimeDefaults = Object.freeze({ liveImagePreviews: true, jobTimeoutMinutes: 180, jobRetentionHours: 3, mediaToolsDirectory: '' });
export const retentionDefaults = Object.freeze({ referenceUploadHours: 24, exportHours: 24 });
export const defaultPromptModel = 'huihui_ai/qwen3-abliterated:4b-instruct-2507-q4_K_M';
export const ollamaAddress = () => 'http://127.0.0.1:11434';
...
export function importEnvironment(database, environment = process.env) {
  ...
  saved = { legacy, runtime: {
    liveImagePreviews: environment.VPIPE_LIVE_PREVIEWS === undefined ? runtimeDefaults.liveImagePreviews : environment.VPIPE_LIVE_PREVIEWS !== '0',
    jobTimeoutMinutes: Number.isFinite(minutes) && minutes > 0 ? Math.min(10080, Math.max(1, Math.ceil(minutes))) : runtimeDefaults.jobTimeoutMinutes,
  } };
...
export function readRuntimeOptions(database, environment = process.env) {
  ...
  const saved = { ...runtimeDefaults, ...migrated.runtime, ...readSetting(database, 'runtimeOptions', {}) };
  // Retired options in older libraries must never re-enable managed services.
  return Object.fromEntries(Object.keys(runtimeDefaults).map(key => [key, saved[key]]));
}
export function readOllamaAddress(database, environment = process.env) {
  const migrated = importEnvironment(database, environment);
  const fallback = ollamaAddress();

...
else if(video){const {width,height,frames,fps}=video.config;const result=spawnSync(resolveMediaTool('ffmpeg'),['-v','error','-y','-f','lavfi','-i',`color=c=0x436450:s=${width}x${height}:r=${fps}`,'-f','lavfi','-i','sine=frequency=220:sample_rate=32000','-t',String(prompt==='short-video'?1:frames/fps),'-c:v','libx264','-preset','ultrafast','-pix_fmt','yuv420p','-c:a','aac',stage('save-video').config.output_url],{encoding:'utf8'});if(result.status){console.error(result.stderr);process.exit(result.status);}}

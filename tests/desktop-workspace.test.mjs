...
const options = { home, templates, groups: [group], version: '0.1.0' };
beforeEach(async () => {
...
test('first boot creates all workspace folders and installs matching pipeline companions', async () => {
  ...
  for (const folder of ['pipelines', 'data', 'logs', 'profile', 'updates']) assert.ok((await fs.stat(result[folder])).isDirectory());
...
test('corrupt installation records fail without replacing pipelines', async () => {
  const result = await ensureWorkspace(options);
  await fs.writeFile(path.join(home, '.pipeline-state.json'), '{broken');
...
test('a missing factory template aborts reset before touching existing pipelines',async()=>{
  ...
  await assert.rejects(fs.stat(path.join(home,'.pipeline-install.lock')),{code:'ENOENT'});
});
